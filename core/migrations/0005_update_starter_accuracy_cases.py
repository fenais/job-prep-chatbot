...
def update_starter_accuracy_cases(apps, schema_editor):
    AccuracyTestCase = apps.get_model("core", "AccuracyTestCase")
    starter_cases_by_name = {
        test_case["name"]: test_case for test_case in STARTER_ACCURACY_TESTS
    }

    for name, starter_case in starter_cases_by_name.items():
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("core", "0004_seed_starter_content"),
    ]

    operations = [
        migrations.RunPython(update_starter_accuracy_cases, migrations.RunPython.noop),
    ]
